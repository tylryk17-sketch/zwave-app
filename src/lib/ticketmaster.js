...
export async function searchEvents({ city = '', keyword = '', category = '', size = 20, page = 0 } = {}) {
  try {
    ...
    const params = new URLSearchParams({
      ...
      ...(city && { city }),
      ...(keyword && { keyword }),
      ...(category ? { classificationName: category } : { segmentName: 'Music' }),
    })
    ...
    const events = (data._embedded?.events || []).filter(e => {
      ...
      if (!venue?.name || venue.name === 'TBD') return false
      if (!venue?.city?.name || venue.city.name === 'TBD') return false
      if (e.classifications?.[0]?.segment?.name === 'Undefined') return false
      return true
    })
...
export async function getTrendingEvents(size = 8) {
  try {
    ...
    const events = (data._embedded?.events || []).filter(e => {
      const venue = e._embedded?.venues?.[0]
      return venue?.name && venue.name !== 'TBD' && venue?.city?.name && venue.city.name !== 'TBD'
    })
    return events.map(formatEvent)
  } catch (err) {
...
function formatEvent(event) {
  ...
  return {
    ...
    vibe: parseFloat((7.5 + Math.random() * 2.5).toFixed(1)),
    ...
    tags: [classification?.segment?.name, classification?.genre?.name, venue?.city?.name].filter(Boolean),
