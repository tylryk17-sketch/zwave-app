  ...
  return (
    <>
      ...
      <nav style={{
        ...
        padding:'0 1.75rem',
        background:'rgba(253,250,245,0.93)', backdropFilter:'blur(20px)',
        borderBottom:'0.5px solid rgba(12,7,2,0.07)'
      }}>
        <button onClick={() => onNavigate('home')} style={{ background:'none', border:'none', fontFamily:"'Playfair Display', serif", fontSize:'19px', fontWeight:700, color:'var(--ink)', cursor:'pointer', letterSpacing:'-0.01em' }}>
          Zwave<sup style={{ fontSize:'9px', color:'var(--gold)', fontFamily:'DM Sans', verticalAlign:'super' }}>®</sup>
        </button>

        <div style={{ display:'flex', gap:'4px', alignItems:'center' }}>
          {[['Discover','discover'],['Create Event','create-event'],['Promoters','promoters'],['Pricing','pricing']].map(([label, page]) => (
            <button key={label} onClick={() => onNavigate(page)}
              style={{ background:'none', border:'none', fontSize:'13px', color:'var(--warm)', padding:'6px 11px', borderRadius:'8px', cursor:'pointer', transition:'all 0.15s' }}
              onMouseEnter={e => { e.target.style.background='var(--fog)'; e.target.style.color='var(--ink)' }}
              onMouseLeave={e => { e.target.style.background='none'; e.target.style.color='var(--warm)' }}
            >{label}</button>
          ))}
        </div>
        ...
        <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
          {/* Cart */}
          <button onClick={() => onNavigate('cart')} style={{ position:'relative', background:'var(--fog)', border:'0.5px solid var(--line)', borderRadius:'100px', padding:'7px 14px', fontSize:'13px', color:'var(--warm)', display:'flex', alignItems:'center', gap:'6px' }}>
            🎟 Cart
          ...
          {user && (
            <div style={{ position:'relative' }}>
              <button onClick={() => { setNotifPanel(!notifPanel); if(!notifPanel) markAllRead() }}
                style={{ position:'relative', background:'var(--fog)', border:'0.5px solid var(--line)', borderRadius:'100px', padding:'7px 12px', fontSize:'16px', color:'var(--warm)', display:'flex', alignItems:'center', cursor:'pointer' }}>
                🔔
          ...
          {user ? (
            <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
              <button onClick={() => onNavigate('dashboard')} style={{ background:'var(--fog)', border:'0.5px solid var(--line)', borderRadius:'100px', padding:'7px 14px', fontSize:'13px', color:'var(--ink)', fontWeight:500 }}>
                {user.role === 'organizer' ? '📊' : user.role === 'promoter' ? '🔗' : '👤'} {user.name.split(' ')[0]}
              </button>
              <button onClick={logout} style={{ background:'none', border:'none', fontSize:'13px', color:'var(--warm)', cursor:'pointer' }}>Log out</button>
            </div>
          ) : (
            <>
              <button onClick={() => onNavigate('login')} style={{ background:'none', border:'none', fontSize:'13px', color:'var(--warm)', cursor:'pointer', padding:'7px 10px' }}>Log in</button>
              <button onClick={() => onNavigate('signup')} style={{ background:'var(--ink)', color:'var(--paper)', fontSize:'13px', fontWeight:500, padding:'8px 18px', borderRadius:'100px', border:'none', cursor:'pointer' }}>Get started</button>
            </>
          )}
        </div>
      </nav>
      ...
      <div style={{ maxWidth:'1200px', margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1rem' }}>
        ...
        <div style={{ display:'flex', gap:'1.5rem', flexWrap:'wrap' }}>
          {['About','Pricing','Promoters','Privacy','Terms','Support'].map(l => (
            <button key={l} onClick={() => onNavigate(l.toLowerCase())} style={{ background:'none', border:'none', fontSize:'12px', color:'rgba(253,250,245,0.18)', cursor:'pointer', transition:'color 0.2s' }}
...
export function Input({ label, type='text', value, onChange, placeholder, required, style={} }) {
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'6px', ...style }}>
      {label && <label style={{ fontSize:'12px', fontWeight:500, color:'var(--warm)', letterSpacing:'0.04em' }}>{label}</label>}
      <input
        type={type} value={value} onChange={onChange} placeholder={placeholder} required={required}
        style={{ background:'var(--paper2)', border:'0.5px solid var(--line2)', borderRadius:'10px', padding:'11px 14px', fontSize:'14px', color:'var(--ink)', outline:'none', transition:'border-color 0.2s', width:'100%' }}
...
export function Select({ label, value, onChange, options, style={} }) {
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'6px', ...style }}>
      {label && <label style={{ fontSize:'12px', fontWeight:500, color:'var(--warm)', letterSpacing:'0.04em' }}>{label}</label>}
      <select value={value} onChange={onChange} style={{ background:'var(--paper2)', border:'0.5px solid var(--line2)', borderRadius:'10px', padding:'11px 14px', fontSize:'14px', color:'var(--ink)', outline:'none', width:'100%', cursor:'pointer' }}>
...
export function Card({ children, style={}, onClick }) {
  return (
    <div onClick={onClick} style={{ background:'var(--paper)', border:'0.5px solid var(--line)', borderRadius:'var(--radius-lg)', overflow:'hidden', transition:'all 0.2s', cursor: onClick ? 'pointer' : 'default', ...style }}
...
export function EventCard({ event, onNavigate }) {
  ...
  return (
    <Card onClick={() => onNavigate('event', event.id)} style={{ position:'relative' }}>
...
export function SectionHeader({ eyebrow, title, subtitle, center=false, light=false }) {
  return (
    <div style={{ marginBottom:'2.5rem', textAlign: center ? 'center' : 'left' }}>
      {eyebrow && <div style={{ fontSize:'10.5px', fontWeight:600, letterSpacing:'0.14em', textTransform:'uppercase', color: light ? 'var(--gold2)' : 'var(--gold)', marginBottom:'0.5rem' }}>{eyebrow}</div>}
