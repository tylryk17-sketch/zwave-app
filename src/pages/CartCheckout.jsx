...
// Simple QR-like visual using a deterministic pattern from ticket data
  ...
  useEffect(() => {
    ...
    for (let row = 0; row < cells; row++) {
      for (let col = 0; col < cells; col++) {
        // Always draw the three finder patterns (corners)
        ...
        if (inTopLeft || inTopRight || inBottomLeft) {
          ...
          const isFinderBorder = (
            ...
            (col === 0 || col === 6 || col === 7 || col === 8) ||
            (row === cells-1 || row === cells-7 || row === cells-8 || row === cells-9) ||
            (col === cells-1 || col === cells-7 || col === cells-8 || col === cells-9)
          )
  ...
  return (
    <div style={{ background:'var(--paper)', border:'0.5px solid var(--line)', borderRadius:'20px', overflow:'hidden', boxShadow:'0 8px 32px rgba(12,7,2,0.08)' }}>
      ...
      <div style={{ padding:'0 1.5rem 1.25rem', display:'flex', gap:'0.5rem' }}>
        ...
        <button onClick={()=>{ navigator.clipboard?.writeText(ticket.confirmationCode).catch(()=>{}) }} style={{ background:'var(--paper2)', color:'var(--ink)', border:'0.5px solid var(--line)', borderRadius:'100px', padding:'10px 14px', fontSize:'13px', cursor:'pointer', fontFamily:'inherit' }}>📋</button>
      </div>
    </div>
  )
}
      ...
      <div style={{ maxWidth:'900px', margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:'2rem', alignItems:'start' }}>
          ...
          <div style={{ background:'var(--paper)', border:'0.5px solid var(--line)', borderRadius:'14px', padding:'1.5rem', position:'sticky', top:'78px' }}>
            ...
            <Btn variant="ember" size="lg" style={{ width:'100%', borderRadius:'12px' }} onClick={() => {
...
export function CheckoutPage({ onNavigate }) {
  ...
  const [form, setForm] = useState({ email: user?.email || '', name: user?.name || '', card:'', expiry:'', cvv:'' })
  const [processing, setProcessing] = useState(false)
  const [tickets, setTickets] = useState([])
  const confirmationRef = useRef(`ZW${Math.random().toString(36).slice(2,8).toUpperCase()}`)
  ...
  const handlePay = (e) => {
    ...
    setProcessing(true)
    setTimeout(() => {
      // Generate tickets for every item in cart
      const generated = []
      cart.forEach(item => {
        for (let i = 0; i < item.qty; i++) {
          generated.push({
            id: `${confirmationRef.current}-${item.eventId}-${i}`,
      ...
      <div style={{ maxWidth:'760px', margin:'0 auto' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:'3rem' }}>
          ...
          <div style={{ display:'inline-flex', alignItems:'center', gap:'8px', background:'rgba(45,158,95,0.08)', border:'0.5px solid rgba(45,158,95,0.2)', borderRadius:'100px', padding:'6px 16px', fontSize:'12px', fontWeight:500, color:'var(--green)' }}>
            ✓ Order #{confirmationRef.current} · ${cartTotal || tickets.reduce((s,t)=>s+t.price,0)}
          </div>
        </div>
        ...
        <div style={{ background:'var(--paper)', border:'0.5px solid var(--line)', borderRadius:'14px', padding:'1.25rem', marginBottom:'2rem' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem' }}>
            {[
              'Your QR code is your entry pass — screenshot it or download the ticket',
              'Show the QR code at the door to be scanned by the organizer',
              'Each QR code is unique and can only be scanned once',
              'Your tickets are also saved in your Zwave dashboard'
            ].map((tip, i) => (
              <div key={i} style={{ display:'flex', gap:'8px', fontSize:'13px', color:'var(--warm)', lineHeight:1.5 }}>
  ...
  return (
    <div style={{ paddingTop:'58px', minHeight:'100svh', background:'var(--paper2)', padding:'3rem 1.75rem' }}>
      <div style={{ maxWidth:'800px', margin:'0 auto' }}>
        ...
        <p style={{ fontSize:'14px', color:'var(--warm)', marginBottom:'2rem' }}>You're almost there — complete your order below.</p>
          ...
          <form onSubmit={handlePay} style={{ display:'flex', flexDirection:'column', gap:'1.5rem' }}>
            ...
            <div style={{ background:'var(--paper)', border:'0.5px solid var(--line)', borderRadius:'14px', padding:'1.5rem' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1rem' }}>
                <h3 style={{ fontSize:'15px', fontWeight:500 }}>Payment</h3>
                <div style={{ fontSize:'12px', color:'var(--warm)' }}>🔒 Stripe</div>
              </div>
              <div style={{ display:'grid', gap:'0.75rem' }}>
                <Input label="Card number" value={form.card} onChange={update('card')} placeholder="1234 5678 9012 3456" required />
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.75rem' }}>
                  <Input label="Expiry" value={form.expiry} onChange={update('expiry')} placeholder="MM / YY" required />
                  <Input label="CVV" value={form.cvv} onChange={update('cvv')} placeholder="123" required />
                </div>
              </div>
              <div style={{ marginTop:'1rem', padding:'0.75rem', background:'var(--paper2)', borderRadius:'8px', fontSize:'12px', color:'var(--warm)', lineHeight:1.5 }}>
                💡 Demo mode — no real payments processed. After paying you'll receive QR code tickets.
              </div>
            </div>
            <Btn variant="ember" size="lg" style={{ borderRadius:'12px' }} disabled={processing}>
